...
from app.router import LLMRouter, accepted_ids
...
def main():
    ...
    for i, row in enumerate(rows, 1):
        decision, ms = router.route(
            row["text"],
            DialogState(session_id=row["id"]),
        )
        raw = [x.scenario_id for x in decision.scenarios]
        pred = accepted_ids(decision)
        ...
        print(
            f"[{i:03}/{len(rows)}] {row['id']} {row['lang']:<5} "
            f"expected={expected} pred={pred} {ms}ms"
        )
    ...
    if latencies:
        print(f"Router median:    {median(latencies):.1f} ms")
